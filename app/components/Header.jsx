...
import { Terminal, LogIn, LogOut, Settings } from "lucide-react";
...
//   const { user, isAdmin, signOut } = useAuth();
  ...
  // LOG 1: See if the user even exists
  console.log("HEADER USER:", user?.email || "No User Found");

  let isAdmin = false;
  if (user) {
    // 1. Try Database Check
    ...
    // 2. Fallback: Check JWT Metadata (from our manual sync earlier)
    const jwtRole = user.user_metadata?.role || user.app_metadata?.role;
    
    console.log("DB Role:", roleData?.role, "Error:", dbError?.message);
